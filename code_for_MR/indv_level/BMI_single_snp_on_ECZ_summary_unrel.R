#The association between single BMI SNPs and Eczema
outcome <- data2$Eczema
...
# Number cases
numbers<-data2[c("rs2836754_C", "Eczema","PartAg_NT3BLQ1", "Sex", "PC1", "PC2", "PC3")]
numbers<-na.omit(numbers)
print(addmargins(table(numbers$Eczema)))
...
#library(tidyverse)
#write_csv(single_snp, path="M:\\Projects\\Ellen\\results\\BMI_single_snp_on_ECZ_summary_unrel.csv", col_names = TRUE)
write.csv(single_snp, file="M:\\Projects\\Ellen\\results\\BMI_single_snp_on_ECZ_summary_unrel_v2.csv", row.names = F, quote = F)
