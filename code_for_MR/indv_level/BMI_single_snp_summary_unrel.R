#The association between single BMI SNPs and BMI
...
snp_reg <- function(i) {
    Res <- summary(lm(outcome ~ i + age + sex + PC1 + PC2 + PC3))
...
a = which(colnames(data2)=="rs977747_T") #7
b = which(colnames(data2)=="rs2836754_C") #102
...
fit <- lm(outcome ~ data2$rs2836754_C + age + sex + PC1 + PC2 + PC3)
print(nobs(fit))
...
#library(tidyverse)
#write_csv(single_snp, path="M:\\Projects\\Ellen\\results\\BMI_single_snp_summary_unrel.csv", col_names = TRUE)
write.csv(single_snp, file="M:\\Projects\\Ellen\\results\\BMI_single_snp_summary_unrel_v2.csv", row.names = F, quote = F)